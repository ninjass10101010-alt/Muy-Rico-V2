...
export async function renderRasterPdf(
  ...
  // Label size: sheet presets dictate cell size; single uses the effective
  // label dimensions (orientation/square swap already applied by the caller).
  const lw = effSheet !== "single" ? layout.labelW : effWIn;
  const lh = effSheet !== "single" ? layout.labelH : effHIn;
      ...
      for (let p = 0; p < pages; p++) {
        ...
        for (let r = 0; r < layout.rows; r++) {
          for (let c = 0; c < layout.cols; c++) {
            ...
            const oy =
              (sheetH / PT_PER_IN - layout.marginTop - (r + 1) * layout.labelH - r * layout.gapV) *
              PT_PER_IN;
