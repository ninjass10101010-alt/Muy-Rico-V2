-- Fix product data: add pack_sizes column, empanadas product, image_urls, pack_sizes data
-- Run:
--   npx wrangler d1 execute muy-rico-orders --local --file=migrations/0018_fix_product_data.sql
--   npx wrangler d1 execute muy-rico-orders --remote --file=migrations/0018_fix_product_data.sql

-- 1. Add pack_sizes column (was missing from schema)
ALTER TABLE products ADD COLUMN pack_sizes TEXT;

-- 2. Add empanadas product
...
-- 3. Set image_url for all existing products (static menu photos)
...
-- 4. Mark featured products for homepage Del Horno
UPDATE products SET featured = 1 WHERE id IN ('prod_conchas','prod_cookie','prod_empanadas','prod_custom_cake');

-- 5. Add pack_sizes (JSON format matching order.html renderProductTile expectations)
...
-- 6. Deactivate prod_cookies_dozen (replaced by pack_sizes on prod_cookie)
